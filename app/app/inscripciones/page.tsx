...
export default function InscripcionesPage() {
  ...
  const recargarEstudiantes = () => {
    const map = new Map(
      getEstudiantesInscritos(null).map((e) => [e.nombre, e])
    );
  ...
  const servicio = useMemo(() => new ServicioInscripciones(), []);

  // Notas: cada estudiante tiene SU propia cadena (módulo "notas").
  const [calEstudiante, setCalEstudiante] = useState('');
  const notas = useScopedChain('notas', calEstudiante || null);
  const servicioCal = useMemo(() => {
    const chain = notas.store.getChain();
    const repo = notas.store.getRepository();
    if (!chain || !repo) return null;
    return new ServicioCalificaciones(chain, repo);
  }, [notas.snapshot, notas.store]);
  ...
  function inscribir(e: React.FormEvent) {
    ...
    const r = servicio.inscribir({
      estudianteNombre: nombre.trim(),
      grado,
      acudienteNombre: acudiente.trim(),
      periodo: periodo.trim(),
    });
    if (r.ok) {
      setMsg(
        `Estudiante ${r.inscripcion?.estudianteNombre} inscrito correctamente en el registro compartido.`
      );
      setError(null);
      setNombre('');
      setAcudiente('');
  ...
  function registrarCal(e: React.FormEvent) {
    ...
    const r = servicioCal.registrarCalificacion(
      {
        estudiante: calEstudiante,
        grado: insc?.grado ?? grado,
        estado: calEstado,
        registradaPor: DOCENTE.id,
        fecha: calFecha,
      },
      rol
    );
    if (r.ok) {
      setCalMsg(
        `Calificación registrada por ${rol.rol} (${rol.id}) en la cadena de notas de ${calEstudiante}.`
      );
      setCalError(null);
      notas.store.refresh();
    } else {
  ...
  function modificarCal(e: React.FormEvent) {
    ...
    const r = servicioCal.modificarCalificacion(
      {
        calificacionHash: modCalHash,
        nuevoEstado: modEstado,
        autorizadoPor: ADMINISTRADOR.id,
        motivo: modMotivo.trim(),
        fecha: modFecha,
      },
      rol
    );
    if (r.ok) {
      setModMsg(
        `Modificación autorizada y registrada en la cadena de notas de ${calEstudiante} (con justificación).`
      );
      setModError(null);
      setModMotivo('');
      notas.store.refresh();
  ...
  return (
    <main className="flex flex-col gap-4 p-6 max-w-3xl">
      <h1 className="text-2xl font-semibold">Kids Zone — Zona inicial</h1>
      <p className="text-sm text-zinc-600">
        Inscribe estudiantes y gestiona sus calificaciones con roles del
        diagrama: el <b>docente</b> registra la nota, la <b>coordinación</b> la
        modifica con justificación y el <b>padre de familia</b> solo consulta.
        Cada estudiante tiene su propia cadena de notas: un cambio solo afecta a
        esa cadena.
      </p>
      <ChainStatus
        status={notas.snapshot.loaded ? notas.status : 'Cargando…'}
        ok={notas.snapshot.validation.ok}
      />
      ...
      <form
        onSubmit={inscribir}
        className="flex flex-col gap-3 max-w-md border rounded p-4"
      >
      ...
      <section className="border rounded p-4 flex flex-col gap-3">
        <div>
          ...
          <p className="text-xs text-zinc-600">
            El rol seleccionado define qué acciones permite el servicio central
            de calificaciones.
          </p>
        </div>
        ...
        {rol.id === DOCENTE.id ? (
          ...
          >
            ...
            <label className="flex flex-col gap-1 text-xs">
              ...
              <select
                value={calEstudiante}
                onChange={(e) => setCalEstudiante(e.target.value)}
                className="border rounded px-2 py-1"
                required
              >
            ...
            <label className="flex flex-col gap-1 text-xs">
              ...
              <select
                value={calEstado}
                onChange={(e) =>
                  setCalEstado(e.target.value as EstadoCalificacion)
                }
                className="border rounded px-2 py-1"
              >
            ...
            <button
              ...
              className="border rounded px-3 py-1 w-fit disabled:opacity-40"
            >
              Registrar calificación
            </button>
            {estudiantes.length === 0 ? (
              <p className="text-xs text-amber-700">
                Primero inscribe al estudiante arriba.
              </p>
            ) : null}
        ...
        {rol.id === ADMINISTRADOR.id ? (
          <form
            onSubmit={modificarCal}
            className="flex flex-col gap-2 border-t pt-3"
          >
            <b>
              Coordinación → Registro de modificación (2. Autoriza · 3.
              Actualiza)
            </b>
            <label className="flex flex-col gap-1 text-xs">
              ...
              >
                ...
                {calificaciones.map((c) => (
                  <option key={c.hash} value={c.hash}>
                    {c.calificacion.estudiante} — {c.calificacion.grado} —
                    vigente: {c.estadoActual}
                  </option>
                ))}
              </select>
            </label>
            <label className="flex flex-col gap-1 text-xs">
              ...
              <select
                value={modEstado}
                onChange={(e) =>
                  setModEstado(e.target.value as EstadoCalificacion)
                }
                className="border rounded px-2 py-1"
              >
                <option value="Aprobado">Aprobado</option>
            ...
            <label className="flex flex-col gap-1 text-xs">
              ...
              <textarea
                ...
                placeholder="Ej: Corrección de digitación"
                className="border rounded px-2 py-1"
                required
              />
            </label>
            ...
            {calificaciones.length === 0 ? (
              <p className="text-xs text-amber-700">
                No hay calificaciones aún; regístralas como docente primero.
              </p>
            ) : null}
            {modMsg ? <p className="text-sm text-green-700">{modMsg}</p> : null}
            {modError ? (
              <p className="text-sm text-red-600">{modError}</p>
            ) : null}
          </form>
        ) : null}
        ...
        {rol.id === 'padre1' ? (
          <div className="border-t pt-3 text-sm">
            ...
            <p className="text-xs text-zinc-600 mt-1">
              Como {rol.rol} ({rol.id}) tu permiso es <b>Solo lectura</b>: no
              puedes registrar ni modificar calificaciones. Revisa la tabla de
              calificaciones vigentes y el detalle de modificación de cada
              estudiante.
            </p>
          </div>
        ) : null}
      ...
      <section className="border rounded p-4 text-sm flex flex-col gap-2">
        ...
        {calificaciones.length === 0 ? (
          <p className="text-xs text-zinc-600">
            Aún no hay calificaciones. El docente puede registrar la primera
            nota.
          </p>
        ) : (
            ...
            {calificaciones.map((c) => (
              ...
              >
                <div className="flex justify-between gap-2">
                  <span>
                    <b>{c.calificacion.estudiante}</b> · {c.calificacion.grado}
                  </span>
                  <span
                    className={`font-medium ${c.estadoActual === 'Aprobado' ? 'text-green-700' : 'text-red-700'}`}
                  >
                    {c.estadoActual}
                  </span>
                </div>
                <span className="text-xs text-zinc-600">
                  Registrada por {c.calificacion.registradaPor} ·{' '}
                  {c.calificacion.fecha} · bloque #{c.index} ·{' '}
                ...
                {c.modificaciones.length > 0 ? (
                  <ul className="text-xs text-amber-700 flex flex-col gap-1">
                    {c.modificaciones.map((m, i) => (
                      <li key={i}>
                        Modificación #{i + 1} → {m.nuevoEstado} · autorizada por{' '}
                        {m.autorizadoPor} el {m.fecha}: «{m.motivo}»
                      </li>
                    ))}
                  </ul>
                ) : null}
      ...
      <section className="border rounded p-3 text-sm">
        <p>
          Estudiantes inscritos: <b>{estudiantes.length}</b> — Bloques en la
          cadena de notas de {calEstudiante || '—'}:{' '}
          <b>{notas.snapshot.blocks.length}</b>
        </p>
        <p className="text-zinc-600">
          Los pagos y certificados de cada estudiante viven en sus propias
          cadenas y se validan por separado.
        </p>
      </section>

      <hr />
      <h2 className="font-medium">
        Historial de notas de {calEstudiante || '—'}
      </h2>
      <ChainList
        blocks={notas.snapshot.blocks}
        annulled={[]}
        onAnnul={() => false}
      />
    </main>
