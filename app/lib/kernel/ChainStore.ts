...
const initial: ChainSnapshot = { blocks: [], validation: { ok: true }, loaded: false, error: null };
...
export class ChainStore {
  ...
  constructor(
    ...
    private genesisText: string,
    private difficulty: string,
    private firstBlockText?: string,
  ) {}
  ...
  protected sync(error: string | null = null): void {
    ...
    this.snapshot = { blocks: this.chain.chain.map((b) => b.toJSON()), validation: this.chain.isValid(), loaded: true, error };
  }
  ensureLoaded(): void {
    ...
    if (saved) {
      ...
      this.chain = new Blockchain(this.genesisText, this.difficulty, this.strategy);
  ...
  getChain(): Blockchain | null { return this.chain; }
  getRepository(): ChainRepository { return this.repository; }
  refresh(): void { if (!this.chain) return; this.sync(); this.emit(); }
  setError(msg: string): void { if (!this.chain) return; this.sync(msg); this.emit(); }
  /** Escritura genérica append-only — usada por todos los dominios */
