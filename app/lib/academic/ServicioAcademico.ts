...
export class ServicioAcademico {
  ...
  emitirReporte(
    data: ConstructorParameters<typeof ReporteProgreso>[0] extends never
      ? never
      : unknown
  ): { ok: boolean; error?: string; bloque?: Bloque } {
    void data;
    return { ok: false, error: 'Usa emitirReporteData' };
  }

  emitirReporteData(reporte: ReporteProgreso): {
    ok: boolean;
    error?: string;
    bloque?: Bloque;
  } {
    ...
    const bloque = this.chain.addBlock(reporte.toBlockData());
    this.repository.save(this.chain.toJSON());
    return { ok: true, bloque: Bloque.desdeBlock(bloque) };
  }
  ...
  generarCertificado(
    reporte: ReporteProgreso,
    bloqueHash: string
  ): Certificado {
    const cert = Certificado.desdeReporte(reporte, bloqueHash);
    this.chain.addBlock(cert.toBlockData());
    this.repository.save(this.chain.toJSON());
    return cert;
  }
}
